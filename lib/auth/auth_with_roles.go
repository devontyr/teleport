...
// NewStream is the scoped equivalent of [ServerWithRoles.NewStream]. Only a subset of watch kinds
// are supported for scoped identities; see [ScopedServerWithRoles.hasWatchPermissionForKind] for
// current restrictions. Unscoped identities are delegated to [ServerWithRoles.NewStream] to
// preserve queue-size tuning for built-in roles such as proxy and node.
...
// NewWatcher is the scoped equivalent of [ServerWithRoles.NewWatcher]. Only a subset of watch kinds
// are supported for scoped identities; see [ScopedServerWithRoles.hasWatchPermissionForKind] for
// current restrictions. Unscoped identities are delegated to [ServerWithRoles.NewWatcher] to
// preserve queue-size tuning for built-in roles such as proxy and node.
...
// Currently only node kind is supported; advanced features such as search-as-roles, preview-as-roles,
// requestable resources, and login inclusion are not yet available for scoped identities.
...
func (a *ScopedServerWithRoles) GetSSHTargets(ctx context.Context, req *proto.GetSSHTargetsRequest) (*proto.GetSSHTargetsResponse, error) {
	...
	// search_as_roles is supported for unscoped callers but not yet for scoped identities.
	// The bifurcation is isolated here; the listing and routing logic below is uniform.
	useSearchAsRoles := false
	lister := a.ListUnifiedResources
	if unscoped, ok := a.UnscopedServerWithRoles(); ok {
		useSearchAsRoles = true
		lister = unscoped.ListUnifiedResources
	}
...
// ListResources is the scoped equivalent of [ServerWithRoles.ListResources].
// Currently only kube_server and kube_cluster resource types are supported;
...
func (a *ScopedServerWithRoles) ListResources(ctx context.Context, req proto.ListResourcesRequest) (*types.ListResourcesResponse, error) {
	...
	if req.RequiresFakePagination() {
		if req.ResourceType != types.KindKubernetesCluster {
			...
			// fake-paginated methods with a kind that does not properly support scoped access.
...
// GetAuthServers reads auth servers. Auth server reads do not enforce scope pinning, ensuring auth
// servers are readable for all identities regardless of pinning — this pattern should not be used
// outside of essential global configuration reads necessary for basic Teleport functionality.
//
...
// ListAuthServers reads a page of auth servers. Auth server reads do not enforce scope pinning,
// ensuring auth servers are readable for all identities regardless of pinning — this pattern should
// not be used outside of essential global configuration reads necessary for basic Teleport
// functionality.
...
// GetProxies reads proxy servers. Proxy reads do not enforce scope pinning, ensuring proxies are
// readable for all identities regardless of pinning — this pattern should not be used outside of
// essential global configuration reads necessary for basic Teleport functionality.
//
...
func (a *ServerWithRoles) ListProxyServers(ctx context.Context, pageSize int, pageToken string) ([]types.Server, string, error) {
	...
	// used by services.ValidateApp.
	return a.ScopedServerWithRoles().ListProxyServers(ctx, pageSize, pageToken)
}

// ListProxyServers reads a page of proxy servers. Proxy reads do not enforce scope pinning,
// ensuring proxies are readable for all identities regardless of pinning — this pattern should not
// be used outside of essential global configuration reads necessary for basic Teleport
// functionality.
...
// Ping does not require special permissions since it only returns basic status
// information. This is an intentional design choice; alternative methods should
// be used for relaying any sensitive information.
...
// GenerateUserCerts is the source of truth for user cert generation. Scoped identities are
// restricted to Kubernetes certificates; unscoped identities support all cert types.
