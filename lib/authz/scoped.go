...
func (a *authorizer) scopedContextForBuiltinRole(ctx context.Context, role ScopedBuiltinRole) (*ScopedContext, error) {
	...
	for sr := range pinning.SystemRoles(role.ScopePin) {
		...
		if err != nil {
			// TODO(fsparshall/scopes): consider permitting individual system roles to fail without breaking the
			// overall checkers. Also, investigate the possibility of initializing checkers lazily.
			return nil, trace.Wrap(err)
		}
